...
class HandDetector:
    ...
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, minTrackCon=0.5):
        ...
        self.detectionCon = detectionCon#最小侦测置信度
        self.minTrackCon = minTrackCon#最小追踪置信度

        self.mpHands = mp.solutions.hands
        self.hands = self.mpHands.Hands(static_image_mode=self.mode, max_num_hands=self.maxHands,
                                        min_detection_confidence=self.detectionCon,
                                        min_tracking_confidence=self.minTrackCon)
        self.mpDraw = mp.solutions.drawing_utils#画出关键点
    ...
    def findHands(self, img, draw=True, flipType=True):
        ...
        self.results = self.hands.process(imgRGB)#图片的输出结果，调用results下的可以查看是否检测到手，以及其他信息
        ...
        print(self.results.multi_handedness)#检测左手0或右手1，若未检测到则为None
        ...
        if self.results.multi_hand_landmarks:#存放的点表示比例
            for handType, handLms in zip(self.results.multi_handedness, self.results.multi_hand_landmarks):
                ...
                xList = []
                yList = []
                for id, lm in enumerate(handLms.landmark):#enumerate()将第一个参数转换为可枚举的对象，第二个参数（默认0）为索引开始值
                    px, py, pz = int(lm.x * w), int(lm.y * h), int(lm.z * w)#比例*图像参数，得到数值坐标，使用int()去掉小数点
                ...
                if draw:
                    self.mpDraw.draw_landmarks(img, handLms,
                                               self.mpHands.HAND_CONNECTIONS)#在img上画出21个点，第三个参数会连接点
